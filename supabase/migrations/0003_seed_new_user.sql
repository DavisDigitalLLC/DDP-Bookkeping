-- DDP Bookkeeping: auto-seed a new user's profile, product lines, chart of
-- accounts, and expense categories the moment they sign up via Supabase Auth.
--
-- security definer is required because this runs from a trigger on
-- auth.users (owned by the supabase_auth_admin role) and needs to write into
-- RLS-protected public tables on the new user's behalf.
...
as $$
declare
  v_digital_mobile_id uuid;
  v_digital_web_id uuid;
begin
  -- ---------------------------------------------------------------------
  -- Product lines: Service Line -> Department -> Product
  -- ---------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------
  -- Chart of Accounts (standard template from spec Section 7.2)
  -- ---------------------------------------------------------------------
  ...
  values
    ...
    (p_user_id, '6040', 'Taxes', 'expense', 'operating_expense', 'debit');

  -- ---------------------------------------------------------------------
  -- Expense categories (with tax deductibility + Schedule C line items)
  -- ---------------------------------------------------------------------
...
begin
  ...
  perform seed_new_user_defaults(new.id);
